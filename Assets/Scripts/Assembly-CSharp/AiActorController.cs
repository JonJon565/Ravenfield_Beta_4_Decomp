...
public class AiActorController : ActorController
{
	...
	private List<Actor> FindPotentialTargets()
	{
		...
		list.RemoveAll((Actor actor) => !HasEffectiveWeaponAgainst(actor));
	...
	private IEnumerator AiVehicle()
	{
		...
		while (true)
		{
			...
			if (actor.CanEnterSeat() && (!actor.fallenOver || actor.inWater) && HasTargetSeat() && Vector3.Distance(actor.CenterPosition(), targetSeat.transform.position) < 4f)
			{
				CancelPath();
		...
		while (true)
		{
			...
			if (IsSquadLeader() && squad.Ready())
			{
				...
				else if (!squad.IsTakingFire())
				{
					...
					else if (!hasPath && !hasFlightTarget)
					{
	...
	private IEnumerator AiTarget()
	{
		...
		while (true)
		{
			...
			if (!HasTarget() && !actor.fallenOver)
			{
				...
				else if (IsSquadLeader() && closestHighlighted != null && !closestHighlighted.IsSeated() && !HasTargetSeat() && !actor.IsSeated() && investigateAction.TrueDone() && !HasCover() && stayInCoverAction.TrueDone())
				{
	...
	private void NewWaypoint(Vector3 origin, Vector3 target)
	{
		...
		Vehicle vehicle = ((!HasTargetSeat()) ? null : targetSeat.vehicle);
		float num = ((!(vehicle != null)) ? 0f : vehicle.pathingRadius);
		Vehicle vehicle2 = null;
		float num2 = 9999999f;
		foreach (Vehicle vehicle3 in ActorManager.instance.vehicles)
		{
			if (!(vehicle3 == vehicle) && !avoidedVehicles.Contains(vehicle3) && vehicle3.ShouldBeAvoided() && vehicle3.CoarseLineOverlap(origin, target, num))
			{
				float num3 = Vector3.Distance(vehicle3.transform.position, actor.Position());
				if (num3 < num2)
				{
					num2 = num3;
					vehicle2 = vehicle3;
		...
		if (vehicle2 != null)
		{
			AvoidVehicle(vehicle2, origin, target, num);
			avoidedVehicles.Add(vehicle2);
		}
	}
	...
	private List<Vehicle> NearbyUnclaimedVehicles()
	{
		...
		list.RemoveAll((Vehicle vehicle) => vehicle.stuck || vehicle.claimedBySquad || vehicle.EmptySeats() < squad.members.Count || Vector3.Distance(vehicle.transform.position, squadPosition) > 150f);
